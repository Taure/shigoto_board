...
mount(_Arg, _Req) ->
    ...
    Layout = {shigoto_board_layout, render, main_content, #{
        active_page => ~"queues", prefix => Prefix, ws_path => <<(arizona_nova:prefix())/binary, "/live">>, arizona_prefix => arizona_nova:prefix()
    }},
...
render(Bindings) ->
    ...
    <div id="{arizona_template:get_binding(id, Bindings)}">
        ...
        <div class="card">
            ...
            <table>
                <thead><tr><th>Queue</th><th>Status</th><th class="text-right">Available</th><th class="text-right">Executing</th><th>Actions</th></tr></thead>
                <tbody>
                    {arizona_template:render_list(fun(Q) ->
                        QName = maps:get(queue, Q),
                        IsPaused = maps:get(QName, Paused, false),
                        StatusBadge = case IsPaused of true -> ~"<span class=\"badge badge-yellow\">paused</span>"; false -> ~"<span class=\"badge badge-green\">active</span>" end,
...
handle_event(~"pause_queue", #{~"queue" := Queue}, View) ->
    ...
    S1 = arizona_stateful:put_binding(paused, Paused#{Queue => true}, State),
    {ok, Queues} = shigoto_dashboard:queue_stats(),
...
handle_event(~"resume_queue", #{~"queue" := Queue}, View) ->
    ...
    State = arizona_view:get_state(View),
    Paused = arizona_stateful:get_binding(paused, State),
    S1 = arizona_stateful:put_binding(paused, maps:remove(Queue, Paused), State),
    {ok, Queues} = shigoto_dashboard:queue_stats(),
    S2 = arizona_stateful:put_binding(queues, Queues, S1),
    {[], arizona_view:update_state(S2, View)};
handle_event(Event, Params, View) ->
    logger:warning(#{msg => ~"Unhandled event", event => Event, params => Params}),
    {[], View}.
